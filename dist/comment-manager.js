...
class CommentManager {
     ...
     */
    async deduplicateCommentsWithAI(proposedComments, existingComments, fileChanges) {
        if (!this.aiProvider || proposedComments.length === 0) {
            logger_1.logger.info('⏭️ Skipping AI deduplication (no AI provider or no comments)');
            return proposedComments;
        }
        logger_1.logger.info(`🧠 Running AI deduplication check on ${proposedComments.length} proposed comments vs ${existingComments.length} existing comments...`);
        try {
            ...
            const deduplicationPrompt = prompt_templates_1.PromptTemplates.buildDeduplicationPrompt(newCommentsForAI, existingComments, fileChanges);
            logger_1.logger.debug('🧠 AI Deduplication Prompt:');
            logger_1.logger.debug(deduplicationPrompt);
            // Call AI for deduplication analysis
            const response = await this.aiProvider.reviewCode(deduplicationPrompt, '', []);
            logger_1.logger.debug(`🧠 AI Deduplication Response: ${JSON.stringify(response, null, 2)}`);
            ...
            if (!deduplicationResult) {
                logger_1.logger.warn('⚠️ Failed to parse AI deduplication response, keeping all comments');
                return proposedComments;
            }
            // Filter out the comments that AI identified as duplicates
            const filteredComments = proposedComments.filter((_, index) => !deduplicationResult.commentsToFilter.includes(index));
            logger_1.logger.info(`✅ AI Deduplication Results:`);
            logger_1.logger.info(`   📥 Proposed: ${proposedComments.length} comments`);
            logger_1.logger.info(`   ✅ Keeping: ${filteredComments.length} comments`);
            logger_1.logger.info(`   🗑️ Filtered: ${deduplicationResult.commentsToFilter.length} duplicates`);
            // Log detailed reasoning
            if (deduplicationResult.reasoning) {
                logger_1.logger.info(`🧠 AI Reasoning:`);
                Object.entries(deduplicationResult.reasoning).forEach(([index, reason]) => {
                    ...
                    logger_1.logger.info(`   ${action} Comment ${commentIndex}: ${comment?.file}:${comment?.line} - ${reason}`);
                });
            }
            // Log which specific comments were filtered out
            if (deduplicationResult.commentsToFilter.length > 0) {
                logger_1.logger.info(`🗑️ Filtered duplicate comments:`);
                deduplicationResult.commentsToFilter.forEach(index => {
                    const comment = proposedComments[index];
                    if (comment) {
                        logger_1.logger.info(`   - ${comment.file}:${comment.line} - "${comment.message}"`);
                    }
                });
            }
            ...
            logger_1.logger.error('❌ AI deduplication failed:', error);
            logger_1.logger.info('⏭️ Continuing with all proposed comments');
            return proposedComments;
        }
    }
    /**
     ...
     */
    parseDeduplicationResponse(aiResponse) {
        try {
            ...
            if (!jsonMatch) {
                logger_1.logger.warn('No JSON found in AI deduplication response');
                return null;
            }
            const jsonStr = jsonMatch[1] || jsonMatch[0];
            const parsed = JSON.parse(jsonStr.trim());
            if (!parsed.commentsToFilter || !Array.isArray(parsed.commentsToFilter)) {
                logger_1.logger.warn('Invalid deduplication response format: missing commentsToFilter array');
                return null;
            }
            return {
            ...
            logger_1.logger.warn('Failed to parse AI deduplication response:', error);
            return null;
        }
    }
    /**
    ...
    async postInlineComments(issues, fileChanges, existingComments, postedComments) {
        ...
        logger_1.logger.info(`📝 Prepared ${proposedComments.length} proposed comments for AI deduplication`);
        // AI DEDUPLICATION LAYER - Filter out duplicates before posting
        const commentsToPost = await this.deduplicateCommentsWithAI(proposedComments, existingComments, fileChanges);
        logger_1.logger.info(`📤 Posting ${commentsToPost.length} AI-filtered comments (filtered out ${proposedComments.length - commentsToPost.length} duplicates)`);
        ...
        for (const commentData of commentsToPost) {
            ...
            const existingComment = this.findBestMatchingComment(file, line, issues[0], existingComments);
            // Log what we're doing
            if (existingComment) {
                logger_1.logger.info(`🔄 Will update existing comment ${existingComment.comment.id} at ${file}:${line}`);
            }
            else {
                logger_1.logger.info(`➕ Will create new comment at ${file}:${line}`);
            }
            try {
        ...
        logger_1.logger.info(`✅ AI-powered comment posting completed successfully`);
    }
    /**
