...
class CommentManager {
    ...
    generateGitHubFileURL(fileName, lineNumber, fileChanges, _postedComments) {
        ...
        logger_1.logger.debug(`🔍 generateGitHubFileURL called: ${fileName}:${lineNumber} (AI diff line)`);
        ...
        if (lineNumber && lineNumber > 0 && fileChanges) {
            ...
            if (fileChange?.patch) {
                ...
                logger_1.logger.debug(`🔢 CONVERSION: AI diff line ${lineNumber} -> Actual file line ${actualFileLineNumber} for ${fileName}`);
                if (actualFileLineNumber && actualFileLineNumber > 0) {
                    ...
                    logger_1.logger.debug(`📁 Generated diff URL: ${diffURL}`);
                    return diffURL;
                }
                else {
                    logger_1.logger.debug(`⚠️ Could not convert AI line ${lineNumber} to file line for ${fileName}`);
                }
            }
            else {
                logger_1.logger.debug(`⚠️ No patch found for ${fileName}`);
            }
        }
        // Fallback: Just link to files page
        const filesURL = `${baseURL}/files`;
        logger_1.logger.debug(`📁 Fallback to files page: ${filesURL}`);
        return filesURL;
    }
    /**
