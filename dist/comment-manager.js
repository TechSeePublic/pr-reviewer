...
class CommentManager {
    ...
    generateGitHubFileURL(fileName, lineNumber, fileChanges, postedComments) {
        ...
        if (lineNumber && postedComments) {
            ...
            if (commentId) {
                logger_1.logger.debug(`✅ Linking to inline comment ${commentId} for ${commentKey}`);
            ...
            for (const [key, commentId] of postedComments.entries()) {
                if (key.startsWith(`${fileName}:`)) {
                    logger_1.logger.debug(`✅ Found related inline comment ${commentId} for ${fileName} (fallback)`);
                    return `${baseURL}#issuecomment-${commentId}`;
                }
            }
        ...
        logger_1.logger.debug(`📁 Linking to file anchor for ${fileName}`);
