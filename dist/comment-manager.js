...
class CommentManager {
     ...
     */
    generateGitHubFileURL(fileName, lineNumber, fileChanges, postedComments) {
        ...
        logger_1.logger.debug(`📋 Available posted comments: ${JSON.stringify(Array.from(postedComments?.entries() || []))}`);
        ...
        if (lineNumber && postedComments) {
            ...
            logger_1.logger.debug(`🔎 Looking for exact match: ${commentKey} -> ${commentId || 'NOT FOUND'}`);
            if (commentId) {
                const commentURL = `${baseURL}#issuecomment-${commentId}`;
                logger_1.logger.debug(`✅ Linking to inline comment ${commentId} for ${commentKey} -> ${commentURL}`);
                return commentURL;
            }
            ...
            for (const [key, commentId] of postedComments.entries()) {
                if (key.startsWith(`${fileName}:`)) {
                    const commentURL = `${baseURL}#issuecomment-${commentId}`;
                    logger_1.logger.debug(`✅ Found related inline comment ${commentId} for ${fileName} (fallback) -> ${commentURL}`);
                    return commentURL;
                }
            }
            logger_1.logger.debug(`⚠️ No inline comment found for ${commentKey}, falling back to file anchor`);
        }
        ...
        logger_1.logger.debug(`📁 Linking to file anchor for ${fileName} -> ${fileURL}`);
        return fileURL;
    }
    /**
     ...
     */
    async postArchitecturalComment(architecturalIssues, fileChanges) {
     ...
     */
    formatArchitecturalCommentBody(architecturalIssues, _fileChanges) {
            ...
            categoryIssues.forEach((issue, index) => {
                ...
                if (issue.relatedFiles && issue.relatedFiles.length > 1) {
                    ...
                    issue.relatedFiles.forEach(file => {
                        const fileURL = this.generateFileAnchorURL(`https://github.com/${this.prContext.owner}/${this.prContext.repo}/pull/${this.prContext.pullNumber}/files`, file);
                        body += `- [${file}](${fileURL})\n`;
                    });
                    body += '\n';
                }
