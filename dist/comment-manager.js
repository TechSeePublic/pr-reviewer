...
class CommentManager {
    ...
    async postInlineComments(issues, fileChanges, existingComments) {
        ...
        for (const [locationKey, locationIssues] of Object.entries(issuesByLocation)) {
            ...
            logger_1.logger.info(`AI reported line: ${originalLine}`);
            ...
            if (!validLocation) {
                logger_1.logger.warn(`❌ Skipping inline comment for ${file}:${originalLine} - no valid location found in PR diff`);
                ...
                if (fileChange?.patch) {
                    ...
                    logger_1.logger.warn(`Available lines for comments: [${validLines.join(', ')}]`);
                    logger_1.logger.warn(`Requested line ${originalLine} is not in the diff`);
                }
                continue;
            }
            logger_1.logger.info(`✅ Valid location found: line ${validLocation.line} (${validLocation.reason})`);
            logger_1.logger.info(`=================================\n`);
            ...
            logger_1.logger.debug(`📍 Sending to GitHub API: line ${validLocation.line} for issue at original line ${originalLine}`);
            ...
            logger_1.logger.info(`🧪 EXPERIMENTAL: Trying line ${adjustedLine} instead of ${validLocation.line} to fix off-by-one`);
            comment.location.line = adjustedLine;
            // Check if we should update existing comment
            const existingComment = existingComments.find(c => c.location.file === file && c.location.line === adjustedLine);
            try {
                logger_1.logger.info(`\n=== POSTING INLINE COMMENT ===`);
                logger_1.logger.info(`File: ${file}`);
                logger_1.logger.info(`AI suggested line: ${originalLine}`);
                logger_1.logger.info(`Validated line: ${validLocation.line}`);
                logger_1.logger.info(`Adjustment reason: ${validLocation.reason}`);
                ...
                logger_1.logger.warn(`Failed to post inline comment for ${file}:${validLocation.line}:`, error);
            }
        }
    }
    /**
    ...
    findValidCommentLocation(file, requestedLine, fileChanges) {
        ...
        if (validLines.includes(requestedLine)) {
            logger_1.logger.debug(`✅ Exact match: line ${requestedLine} is in diff`);
            return { line: requestedLine, reason: 'exact_match' };
        }
        logger_1.logger.debug(`❌ Line ${requestedLine} not in diff, looking for nearby lines...`);
        ...
        if (closestLine !== null) {
            logger_1.logger.info(`🔄 Adjusted comment location from line ${requestedLine} to ${closestLine} (distance: ${minDistance})`);
            return { line: closestLine, reason: 'nearby_match' };
        }
        logger_1.logger.debug(`❌ No lines within ${maxDistance} distance of ${requestedLine}`);
        ...
        if (requestedLine <= 10) {
            ...
            logger_1.logger.info(`📌 Using first valid line ${firstValidLine} for file-level issue at line ${requestedLine}`);
            return { line: firstValidLine, reason: 'file_level_fallback' };
        }
        logger_1.logger.warn(`❌ No suitable comment location found for ${file}:${requestedLine}`);
