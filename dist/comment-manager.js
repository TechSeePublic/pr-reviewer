...
class CommentManager {
    ...
    async postInlineComments(issues, fileChanges, existingComments) {
        ...
        for (const [locationKey, locationIssues] of Object.entries(issuesByLocation)) {
            ...
            logger_1.logger.debug(`📍 Original calculation: line ${validLocation.line} for issue at AI-reported line ${originalLine}`);
            // Check if we should update existing comment
            const existingComment = existingComments.find(c => c.location.file === file && c.location.line === validLocation.line);
            try {
