...
class CommentManager {
    constructor(githubClient, inputs, aiProvider) {
        ...
        this.inputs = inputs;
        this.flowDiagramGenerator = new flow_diagram_generator_1.FlowDiagramGenerator({}, aiProvider);
    }
    /**
    ...
    async postReviewComments(reviewResult, fileChanges, prPlan) {
        ...
        if (shouldPostSummary) {
            ...
            if (filteredIssues.length > 0 || reviewResult.issues.length === 0) {
                await this.postSummaryComment(summaryResult, fileChanges, existingComments.summaryComment, prPlan);
            }
        }
    }
    /**
