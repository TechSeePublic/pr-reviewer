...
class CommentManager {
    ...
    async postInlineComments(issues, fileChanges, existingComments) {
        ...
        for (const [locationKey, locationIssues] of Object.entries(issuesByLocation)) {
            ...
            try {
                logger_1.logger.info(`Posting inline comment at ${file}:${validLocation.line} (originally ${originalLine})`);
    ...
    findValidCommentLocation(file, requestedLine, fileChanges) {
        ...
        if (requestedLine <= 5) { // Only for issues near top of file
    ...
    parseValidLinesFromPatch(patch) {
        ...
        logger_1.logger.debug(`Valid comment lines for patch: ${validLines.join(', ')}`);
        return validLines;
    }
    /**
