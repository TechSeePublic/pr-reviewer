...
class CommentManager {
    ...
    async postReviewComments(reviewResult, fileChanges, prPlan) {
        ...
        if (shouldPostSummary) {
            ...
            logger_1.logger.info(`Summary comment decision: criticalIssues=${criticalIssues.length}, totalIssues=${reviewResult.issues.length}`);
            ...
            logger_1.logger.info(`Summary shouldPost decision: ${shouldPost} (criticalIssues=${criticalIssues.length}, totalIssues=${reviewResult.issues.length})`);
            if (shouldPost) {
                logger_1.logger.info('Attempting to post summary comment...');
                try {
                ...
                logger_1.logger.info('❌ Summary comment skipped due to shouldPost=false');
            }
        }
        else {
