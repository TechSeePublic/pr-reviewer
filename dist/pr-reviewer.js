...
class PRReviewer {
    constructor(inputs, workspacePath = process.cwd()) {
        ...
        this.aiProvider = factory_1.AIProviderFactory.create(inputs);
        this.autoFixManager = new auto_fix_manager_1.AutoFixManager(this.githubClient, inputs, this.prContext, this.workspacePath);
        this.commentManager = new comment_manager_1.CommentManager(this.githubClient, inputs, this.aiProvider, this.prContext, this.autoFixManager);
    }
    /**
     * Main review process
     */
    async reviewPR() {
        try {
            ...
            let architecturalIssues = [];
            if (this.inputs.enableArchitecturalReview) {
                core.info('🏗️ Conducting architectural review...');
                const architecturalResult = await this.conductArchitecturalReview(fileChanges, applicableRules);
    ...
    extractPRContext() {
        ...
        if (context.eventName === 'workflow_dispatch' || this.inputs.prNumber) {
            ...
            if (!prNumber) {
                throw new Error('PR number is required when running manually. Use workflow_dispatch with pr_number input.');
            }
            return {
        ...
        if (!context.payload.pull_request) {
            throw new Error('This action can only be run on pull request events or manual workflow dispatch with pr_number');
        }
        return {
            owner: context.repo.owner,
            repo: context.repo.repo,
     ...
     */
    async conductArchitecturalReview(fileChanges, rules) {
        try {
     ...
     */
    async reviewFilesInBatches(fileChanges, rules, prPlan, existingComments) {
        const allIssues = [];
