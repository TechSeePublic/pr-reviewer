...
- **Security**: Apply language-specific security best practices`;
     ...
     */
    static buildUserPrompt(context, _code) {
        return `${context}
...
Keep it professional and concise. Only report actual problems.`;
     ...
     */
    static buildReviewContext(fileChange, fileContent) {
...
`;
        ...
        if (fileChange.patch) {
            ...
            logger_1.logger.info(`Processing numbered diff for ${fileChange.filename} (${numberedDiff.split('\n').length} lines)`);
...
`;
     ...
     */
    static createNumberedDiff(patch) {
...
Focus on understanding the **intent**, **impact**, and **broader system implications** of these changes rather than line-by-line details.`;
     ...
     */
    static extractCodeContext(file, lineNumber, contextLines = 3) {
        if (!file.patch)
     ...
     */
    static buildBatchReviewPrompt(files, rules, prPlan, existingComments) {
...
`;
            if (file.patch) {
                ...
                // Debug: Log basic diff info without full content
                logger_1.logger.info(`Processing numbered diff for ${file.filename} in batch mode (${numberedDiff.split('\n').length} lines)`);
...
**Final Reminder**: In your JSON response, ensure every issue has a "file" property set to one of the exact filenames listed above.`;
     ...
     */
    static buildDeduplicationPrompt(newComments, existingComments, fileChanges) {
...
Analyze now and return the JSON response:`;
     ...
     */
    static buildArchitecturalReviewPrompt(files, rules, config) {
        const jsonInstructions = config.supportsJsonMode
            ? 'Return your response as a valid JSON object only.'
            : 'Return your response as a valid JSON object only. Start your response with { and end with }.';
...
`;
            if (file.patch) {
                ...
                const diffPreview = numberedDiff.length > 3000
