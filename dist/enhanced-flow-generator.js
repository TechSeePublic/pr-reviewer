...
exports.EnhancedFlowGenerator = void 0;
const logger_1 = require("./logger");
class EnhancedFlowGenerator {
     ...
     */
    async generateFlowCharts(fileChanges, prPlan, _issues = []) {
        try {
            logger_1.logger.info('Generating enhanced flow charts for PR...');
            // Filter relevant files for flow analysis
            const relevantFiles = this.filterRelevantFiles(fileChanges);
            if (relevantFiles.length === 0) {
                logger_1.logger.info('No relevant files found for flow chart generation');
                return null;
            }
            ...
            logger_1.logger.error('Failed to generate enhanced flow charts:', error);
            return null;
        }
    }
    /**
     ...
     */
    async analyzeBusinessFlow(files, prPlan) {
        ...
        if (this.aiProvider) {
            try {
                ...
                logger_1.logger.warn('AI business flow analysis failed, using fallback:', error);
            }
        }
     ...
     */
    async getAIBusinessFlow(files, prPlan) {
        if (!this.aiProvider)
            return [];
        const prompt = `
...
`;
        try {
            ...
            logger_1.logger.error('AI business flow generation failed:', error);
            return [];
        }
    }
    /**
     * Parse AI response for business steps
     */
    parseAIBusinessResponse(response) {
        try {
            ...
            logger_1.logger.warn('Failed to parse AI business response:', error);
        }
        return [];
    }
    /**
     ...
     */
    generateBusinessFlowFallback(files, prPlan) {
        const steps = [];
        // Analyze file types to determine business flow
        const hasUI = files.some(f => f.filename.includes('component') ||
            f.filename.includes('Component') ||
            f.filename.endsWith('.tsx') ||
            f.filename.endsWith('.jsx') ||
            f.filename.endsWith('.vue'));
        const hasAPI = files.some(f => f.filename.includes('api') ||
            f.filename.includes('endpoint') ||
            f.filename.includes('route'));
        const hasService = files.some(f => f.filename.includes('service') ||
            f.filename.includes('Service') ||
            f.filename.includes('manager') ||
            f.filename.includes('handler'));
        const hasDatabase = files.some(f => f.filename.includes('model') ||
     ...
     */
    async analyzeCodeFlow(files, prPlan) {
        const flows = [];
    ...
    groupFilesByType(files) {
        ...
        for (const file of files) {
            ...
            if (filename.endsWith('.tsx') || filename.endsWith('.jsx') || filename.endsWith('.vue') || filename.includes('component')) {
                groups.frontend.push(file);
            }
            else if (filename.includes('api') || filename.includes('endpoint') || filename.includes('route')) {
                groups.api.push(file);
            }
            else if (filename.includes('service') || filename.includes('manager') || filename.includes('handler')) {
                groups.service.push(file);
            }
            else if (filename.includes('model') || filename.includes('schema') || filename.includes('db')) {
                groups.database.push(file);
            }
            else if (filename.includes('util') || filename.includes('helper') || filename.includes('config')) {
     ...
     */
    createCodeFlowForType(type, files, prPlan) {
        if (files.length === 0)
            return null;
     ...
     */
    inferActionsFromFiles(files, prPlan) {
        const actions = [];
        const overview = prPlan.overview.toLowerCase();
     ...
     */
    getComponentDescription(type, actions) {
     ...
     */
    generateIntegrationFlows(filesByType) {
        const flows = [];
        // Frontend -> API integration
        if (filesByType.frontend && filesByType.api && filesByType.frontend.length > 0 && filesByType.api.length > 0) {
            flows.push({
                ...
                type: 'frontend',
                files: [...filesByType.frontend.map(f => f.filename), ...filesByType.api.map(f => f.filename)],
                dependencies: ['api']
            });
        }
        // API -> Service integration
        if (filesByType.api && filesByType.service && filesByType.api.length > 0 && filesByType.service.length > 0) {
            flows.push({
                ...
                type: 'api',
                files: [...filesByType.api.map(f => f.filename), ...filesByType.service.map(f => f.filename)],
                dependencies: ['service']
            });
        }
        // Service -> Database integration
        if (filesByType.service && filesByType.database && filesByType.service.length > 0 && filesByType.database.length > 0) {
            flows.push({
                ...
                type: 'service',
                files: [...filesByType.service.map(f => f.filename), ...filesByType.database.map(f => f.filename)],
     ...
     */
    generateCombinedMermaid(businessSteps, codeFlows) {
        let mermaid = 'flowchart TD\n';
        // Add title
     ...
     */
    getCodeNodeShape(flow) {
        const component = flow.component.length > 20 ? flow.component.substring(0, 17) + '...' : flow.component;
        switch (flow.type) {
            case 'frontend':
                return `[["🎨 ${component}"]]`;
            case 'api':
    ...
    filterRelevantFiles(fileChanges) {
        return fileChanges.filter(file => {
            ...
            const excludePatterns = [
                ...
                '.gitignore', '.eslintrc', 'readme.md', 'license',
     ...
     */
    generateDescription(businessFlow, codeFlow) {
...
exports.EnhancedFlowGenerator = EnhancedFlowGenerator;
//# sourceMappingURL=enhanced-flow-generator.js.map