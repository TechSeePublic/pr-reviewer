...
class FlowDiagramGenerator {
    ...
    filterRelevantFiles(fileChanges) {
        return fileChanges.filter(file => {
            ...
            const hasRelevantExtension = this.config.includeFileTypes.some(ext => file.filename.toLowerCase().includes(ext));
            if (!hasRelevantExtension)
                return false;
            // Exclude test files and other irrelevant files
            const isExcluded = this.config.excludeFileTypes.some(pattern => file.filename.toLowerCase().includes(pattern));
            if (isExcluded)
                return false;
