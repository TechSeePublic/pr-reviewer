...
class FlowDiagramGenerator {
     ...
     */
    async generateAIDiagram(files, prPlan, prType, visualizationType) {
        logger_1.logger.info(`Generating AI ${visualizationType} diagram for ${prType} PR...`);
        try {
    ...
    async generateMermaidWithAI(prompt, context) {
        ...
        try {
            ...
            logger_1.logger.info(`Flow diagram generation: provider=${providerName}, model=${providerModel}`);
            ...
            logger_1.logger.info('✅ Got AI response for Mermaid generation');
            return response;
        }
        catch (error) {
            logger_1.logger.error('AI provider call failed:', error);
            // Provide more detailed error information
            if (error && typeof error === 'object') {
                const errorObj = error;
                if (errorObj.message) {
                    logger_1.logger.error(`AI Error Message: ${errorObj.message}`);
                }
                if (errorObj.status) {
                    logger_1.logger.error(`AI Error Status: ${errorObj.status}`);
                }
                if (errorObj.code) {
                    logger_1.logger.error(`AI Error Code: ${errorObj.code}`);
                }
            }
            throw error;
        }
     ...
     */
    async callAIDirectly(prompt, context) {
        if (!this.aiProvider) {
            throw new Error('AI provider not available');
        }
...
- Your entire response should be pure Mermaid code only`;
        ...
        if (this.aiProvider.name === 'openai' && 'client' in this.aiProvider) {
            ...
            const azureProvider = this.aiProvider;
            const requestConfig = {
            ...
            // Anthropic provider
            const anthropicProvider = this.aiProvider;
            ...
            // Bedrock provider
            const bedrockProvider = this.aiProvider;
            ...
            logger_1.logger.warn(`Unknown provider type: ${this.aiProvider.name}, falling back to reviewCode method`);
     ...
     */
    selectVisualizationType(prType, files, prPlan) {
        const overview = prPlan.overview.toLowerCase();
        const keyChanges = prPlan.keyChanges.join(' ').toLowerCase();
        const fileNames = files.map(f => f.filename.toLowerCase()).join(' ');
        switch (prType) {
            ...
            case 'optimization':
                ...
                if (overview.includes('performance') ||
            ...
            case 'maintenance':
                ...
                if (overview.includes('dependency') ||
     ...
     */
    buildSpecializedPrompt(files, prPlan, prType, visualizationType) {
        switch (prType) {
            case 'feature':
     ...
     */
    buildFeaturePrompt(files, prPlan, visualizationType) {
...
Return only the Mermaid ${visualizationType} code with creative, PR-specific branching logic.`;
     ...
     */
    buildBugfixPrompt(files, prPlan, visualizationType) {
        const fileList = files.map(f => `- ${f.filename} (${f.status})`).join('\n');
...
Return only the Mermaid flowchart code with creative, bug-specific branching logic.`;
     ...
     */
    buildOptimizationPrompt(files, prPlan, _visualizationType) {
        const fileList = files.map(f => `- ${f.filename} (${f.status})`).join('\n');
...
Return only the Mermaid flowchart code with creative, optimization-specific branching logic.`;
     ...
     */
    buildRefactorPrompt(files, prPlan, _visualizationType) {
        const fileList = files.map(f => `- ${f.filename} (${f.status})`).join('\n');
...
Return only the Mermaid flowchart code that shows the improved structure.`;
     ...
     */
    buildMaintenancePrompt(files, prPlan, _visualizationType) {
        const fileList = files.map(f => `- ${f.filename} (${f.status})`).join('\n');
...
Return only the Mermaid flowchart code that shows the maintenance updates.`;
     ...
     */
    buildGenericPrompt(files, prPlan, _visualizationType) {
        const fileList = files.map(f => `- ${f.filename} (${f.status})`).join('\n');
...
`;
    ...
    parseMermaidResponse(response) {
        ...
        if (typeof response === 'string') {
            ...
            if (extracted) {
                ...
                if (response.trim().startsWith('flowchart')) {
                    logger_1.logger.info('✅ Using response directly as it starts with flowchart');
        ...
        // Handle array response (from reviewCode fallback)
            ...
            if (isCodeReviewResponse) {
                logger_1.logger.warn('AI returned code review issues instead of Mermaid diagram - extracting from message fields');
                // Try to extract from code review response fields
                for (const item of response) {
                    if (item.description || item.message) {
                        const text = item.description || item.message;
                        logger_1.logger.info(`Trying to extract Mermaid from code review item: ${text.substring(0, 150)}...`);
                        const extracted = this.extractMermaidFromText(text);
                        if (extracted) {
                            logger_1.logger.info('✅ Successfully extracted Mermaid code from code review item');
                            mermaidCode = extracted;
                            break;
                        }
                    }
                ...
                if (!mermaidCode) {
                    logger_1.logger.error('AI returned code review format and no Mermaid could be extracted');
        ...
        if (!mermaidCode || !mermaidCode.includes('flowchart')) {
            ...
            logger_1.logger.error(`Final mermaidCode length: ${mermaidCode?.length || 0} chars`);
    ...
    isValidMermaidCode(code) {
        ...
        if (nodeWithParens) {
            logger_1.logger.warn('Mermaid validation failed: node labels contain parentheses which break syntax');
            return false;
        }
        ...
        for (const line of codeLines2) {
            ...
            if (nodeMatch && nodeMatch[1]) {
                ...
                if (problematicChars.test(nodeText)) {
                    logger_1.logger.warn(`Mermaid validation failed: node label "${nodeText}" contains problematic characters`);
                    return false;
                }
            }
        }
        ...
        if (nodeCount < 5) {
            logger_1.logger.warn(`Mermaid validation failed: diagram has only ${nodeCount} steps, minimum 5 required for meaningful flow`);
            return false;
        }
    ...
    hasExplanatoryValue(mermaidCode) {
        ...
        if (!hasDecisions) {
            logger_1.logger.warn('Mermaid validation failed: diagram lacks decision points (diamond shapes {})');
            return false;
        }
        if (decisionCount < 2) {
            logger_1.logger.warn(`Mermaid validation failed: diagram has only ${decisionCount} decision points, minimum 2 required for proper branching`);
            return false;
        }
        if (!hasConditionalFlows) {
            logger_1.logger.warn('Mermaid validation failed: diagram lacks conditional arrows (-->|condition|)');
            return false;
        }
        if (conditionalCount < 2) {
            logger_1.logger.warn(`Mermaid validation failed: diagram has only ${conditionalCount} conditional flows, minimum 2 required for meaningful branching`);
            return false;
        }
        ...
        if (arrowCount <= nodeCount) {
            logger_1.logger.warn(`Mermaid validation failed: diagram appears too linear (${arrowCount} arrows for ${nodeCount} nodes). Need more branching.`);
            return false;
        }
        return true;
    }
     ...
     */
    generateSmartDescription(prPlan, fileCount, prType, visualizationType) {
        let description = '';
     ...
     */
    getEnhancedExampleDiagram(visualizationType, prType) {
...
**BE CREATIVE:** Show the actual component relationships changed in this PR.`;
     ...
     */
    getExampleDiagram(visualizationType, _prType) {
        switch (visualizationType) {
            case 'flowchart':
                return `flowchart TD
     ...
     */
    getVisualizationDisplayName(visualizationType) {
        switch (visualizationType) {
            case 'flowchart':
                return 'flowchart';
            case 'graph':
