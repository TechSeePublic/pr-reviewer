...
class AzureOpenAIProvider {
    ...
    async detectModelCapabilities() {
        ...
        if (isKnownReasoningModel) {
            ...
            logger_1.logger.info(`✅ Model ${this.model} recognized as reasoning model by name pattern`);
            return;
        }
        ...
        try {
            ...
            try {
                ...
                logger_1.logger.info(`✅ Model ${this.model} confirmed to use max_tokens parameter`);
            }
            catch (error) {
                ...
                if (errorMessage.includes('max_tokens') && errorMessage.includes('max_completion_tokens')) {
                    ...
                    this.modelCapabilities.requiresMaxCompletionTokens = true;
                    logger_1.logger.info(`✅ Model ${this.model} confirmed to require max_completion_tokens parameter`);
            ...
            logger_1.logger.warn(`⚠️ Detection failed for ${this.model}, will try both parameters dynamically`);
        }
    }
    ...
    async makeRequestWithFallback(requestConfig) {
            ...
            try {
                ...
                const configWithTokens = {
                    ...
                    ...(usesMaxCompletionTokens
                        ? { max_completion_tokens: requestConfig.max_tokens || requestConfig.max_completion_tokens }
                ...
                if (errorMessage.includes('max_tokens') && errorMessage.includes('max_completion_tokens')) {
                    ...
                    this.modelCapabilities.requiresMaxCompletionTokens = !this.modelCapabilities.requiresMaxCompletionTokens;
                    ...
                    logger_1.logger.info(`✅ Found correct parameter for ${this.model}: ${usesMaxCompletionTokens ? 'max_completion_tokens' : 'max_tokens'}`);
                    return await this.client.chat.completions.create(configWithTokens);
                }
                else {
