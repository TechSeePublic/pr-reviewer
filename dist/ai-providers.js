        ...
        for (const file of files) {
            ...
            if (fileName.includes('.tsx') || fileName.includes('.jsx')) {
                if (lowerMessage.includes('react') || lowerMessage.includes('component') ||
                    lowerMessage.includes('jsx') || lowerMessage.includes('hook')) {
                    return file;
                }
            }
            ...
            if (fileName.includes('api') || fileName.includes('server') || fileName.includes('route')) {
                if (lowerMessage.includes('api') || lowerMessage.includes('endpoint') ||
                    lowerMessage.includes('route') || lowerMessage.includes('server')) {
                    return file;
                }
            }
            // Test file patterns
...
class AzureOpenAIProvider {
    constructor(apiKey, endpoint, apiVersion, model, realModel) {
        this.name = 'azure';
...
class AIProviderFactory {
    static create(inputs) {
        ...
        if (provider === 'azure') {
            ...
            return new AzureOpenAIProvider(inputs.azureOpenaiApiKey, inputs.azureOpenaiEndpoint, inputs.azureOpenaiApiVersion || '2024-10-21', model, inputs.azureOpenaiRealModel);
        }
