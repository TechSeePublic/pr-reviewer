...
function getActionInputs() {
    ...
    const inputs = {
        ...
        inlineSeverity: core.getInput('inline_severity') || 'error',
        summaryFormat: core.getInput('summary_format') || 'detailed',
        logLevel: core.getInput('log_level') || 'error',
...
function validateInputs(inputs) {
    ...
    if (inputs.aiProvider === 'bedrock' && !inputs.bedrockRegion) {
        throw new Error('Bedrock region is required when ai_provider is set to "bedrock"');
    }
    if (inputs.aiProvider === 'bedrock' && !inputs.bedrockApiKey && (!inputs.bedrockAccessKeyId || !inputs.bedrockSecretAccessKey)) {
        throw new Error('Bedrock authentication is required: either provide bedrock_api_key OR both bedrock_access_key_id and bedrock_secret_access_key');
    }
    if (inputs.aiProvider === 'auto' &&
        ...
        throw new Error('At least one AI provider configuration is required (openai_api_key, anthropic_api_key, azure_openai_api_key, or bedrock_region)');
    }
