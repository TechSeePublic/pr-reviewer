...
import { AIResponse, ArchitecturalReviewResult, CodeIssue, FileChange, PRPlan } from '../types';
...
export class AIProviderUtils {
  ...
  static extractErrorMessage(error: unknown): string {
    ...
    if (error && typeof error === 'object') {
      if ('message' in error) {
        ...
        errorMessage = String((error.error as Record<string, unknown>).message);
      }
    }
    return errorMessage;
  }
   ...
   */
  static parseAIResponse(response: string, deterministicMode: boolean = false): CodeIssue[] {
    try {
